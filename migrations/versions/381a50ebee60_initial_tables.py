"""Initial tables

Revision ID: 381a50ebee60
Revises: 
Create Date: 2026-02-06 15:18:02.560111

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '381a50ebee60'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('users',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('username', sa.String(length=64), nullable=False),
    sa.Column('email', sa.String(length=120), nullable=False),
    sa.Column('password_hash', sa.String(length=128), nullable=False),
    sa.Column('visit_count', sa.Integer(), nullable=True),
    sa.Column('last_login_date', sa.Date(), nullable=True),
    sa.Column('last_login_time', sa.Time(), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('email'),
    sa.UniqueConstraint('username')
    )
    op.create_table('water_data',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.Column('project_type', sa.String(length=64), nullable=False),
    sa.Column('water_type', sa.String(length=64), nullable=False),
    sa.Column('date', sa.Date(), nullable=False),
    sa.Column('time', sa.Time(), nullable=False),
    sa.Column('latitude', sa.Float(), nullable=False),
    sa.Column('longitude', sa.Float(), nullable=False),
    sa.Column('pin_id', sa.String(length=32), nullable=False),
    sa.Column('image_path', sa.String(length=256), nullable=True),
    sa.Column('temperature', sa.Float(), nullable=True),
    sa.Column('pH', sa.Float(), nullable=True),
    sa.Column('DO', sa.Float(), nullable=True),
    sa.Column('TDS', sa.Float(), nullable=True),
    sa.Column('chlorophyll', sa.Float(), nullable=True),
    sa.Column('TA', sa.Float(), nullable=True),
    sa.Column('DIC', sa.Float(), nullable=True),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('water_data')
    op.drop_table('users')
    # ### end Alembic commands ###
